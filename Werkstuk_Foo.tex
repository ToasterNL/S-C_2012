...
\begin{prooftree}
	...
	\UnaryInfC{$\langle \&3,  (\sigma, AV, \rho, \theta, O) \rangle \rightarrow (\sigma, AV[\sigma \mapsto 3], \rho, \theta, O)$}
		...
		\RightLabel{$[Loop_{ns}^{tt^1}]$}	
		\BinaryInfC{$\langle (0\#1-1\$i\$c10),  (\sigma, AV[\sigma \mapsto 3], \rho, \theta, O) \rangle \rightarrow s_{12}$}	
			
				\AxiomC{$T_4$}

		\RightLabel{$[Comp_{ns}]$}
		\BinaryInfC{$\langle (0\#1-1\$i\$c10)''boom!''\$c10,  (\sigma, AV[\sigma \mapsto 3], \rho, \theta, O) \rangle \rightarrow s_{14}$}
	\RightLabel{$[Comp_{ns}]$}
