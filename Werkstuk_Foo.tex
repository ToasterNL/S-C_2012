...
\begin{prooftree}
			...
			\BinaryInfC{$\langle \#1-1\$i\$c10, (\sigma, AV[\sigma \mapsto 3], \rho, \theta, O) \rangle \rightarrow (s= nog niet bekend 2) $}
\end{prooftree}
\end{figure}
...
\begin{prooftree}
	...
	\UnaryInfC{$\langle \&3,  (\sigma, AV, \rho, \theta, O) \rangle \rightarrow (\sigma, AV[\sigma \mapsto 3], \rho, \theta, O)$}
			...
			\AxiomC{$\langle (0\#1-1\$i\$c10)''boom!''\$c10, (s= nog niet bekend 2) \rangle \rightarrow (s = nog niet bekend)$}
		
		\RightLabel{$[Loop_{ns}^{tt^1}]$}
		\BinaryInfC{$\langle (0\#1-1\$i\$c10)''boom!''\$c10, (\sigma , AV, \rho , \theta, O) \rangle \rightarrow (s=nog niet bekend)$}
	\RightLabel{$[Comp_{ns}]$}
