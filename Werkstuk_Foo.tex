...
\caption{$T_6$
		...
		\\$s_{1}: (\sigma, AV, \rho+\Delta, 1\|\theta, O)$
		\\$s_{2}: (\sigma+1, AV, \rho+2\Delta, 1\|\theta, O)$
		\\$s_{3}: (\sigma+1, AV[\sigma \mapsto \theta[0]], \rho+3\Delta, \theta[1...], O)$
		\\$s_{3}: (\sigma+1, AV[\sigma \mapsto 1], \rho+4\Delta, \theta, O)$
		\\$s_{4}: (\sigma+1, AV[\sigma \mapsto 1], \rho+5\Delta, \theta, O \| $"Stack is empty"$)$
}
%\centering
\begin{prooftree}
	\AxiomC{$\langle @1, s \rangle \rightarrow s_1 $}
		\AxiomC{$\langle >, s_1 \rangle \rightarrow s_2 $}
			\AxiomC{$\langle \&, s_2 \rangle \rightarrow s_3 $}
				\AxiomC{$\langle \&, s_3 \rangle \rightarrow s_4 $}
				\RightLabel{$[Comp_{ns}]$}
