...
Een aantal constructies die interessant zijn, zoals de pauzeer (\#) operatie, gebruik van een standard output en foutafhandeling, zullen we in dit hoofdstuk bespreken. %als inherent taalonderdeel, in tegenstelling tot While dat stacks slechts als intern berekeningsmodel gebruikt.
...
\begin{tabular}{ | l | p{12cm} |}
    ...
    $undef$    & Met dit symbool bedoelen we een ongedefinieerde waarde, zoals bij een fout als resultaat kan optreden. \\ \hline
...
%De stack is zo dus gepopt, en in de volgende regels die de stack poppen zal ditzelfde principe worden gebruikt.
\newline
\newline
%\begin{table}[h]
...
$[print^h_{ns}]$	&	\AxiomC{$\langle $\$h$, (\sigma, AV, \rho, \theta, O) \rangle \rightarrow  (\sigma, AV, \rho+\Delta, \theta, O \| hex(AV[\sigma]))$}
				\DisplayProof
...
$[print^{hn}_{ns}]$	&	\AxiomC{$\langle $\$h$n, (\sigma, AV, \rho, \theta, O) \rangle \rightarrow  (\sigma, AV, \rho+\Delta, \theta, O \| hex(n))$}
					\DisplayProof
...
\caption{Navigatie voorbeeld
		...
		\\$s_{4}: (1, AV[\sigma \mapsto undef], 4\Delta, \theta, O \| $Stack is empty$ \| newline)$
}
%\centering
\begin{prooftree}
	\AxiomC{}
