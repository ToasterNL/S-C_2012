...
Een aantal constructies die interessant zijn, zoals de pauzeer (\#) operatie en foutafhandeling, zullen we in dit hoofdstuk bespreken. %als inherent taalonderdeel, in tegenstelling tot While dat stacks slechts als intern berekeningsmodel gebruikt.
...
Om dit te simuleren moeten we ervoor zorgen dat iedere operatie een bepaalde tijd kost, waarbij pauzeer-operatie een opgegeven aantal seconden duurt.
...
Door het nagaan van de referentie-implementatie van Foo hebben we geconstateerd dat in dit geval, bij het poppen van de stack namelijk de waarde nul wordt teruggegeven.
We hebben in deze foutafhandeling echter een inconsistentie in de referentie-implementatie ontdekt, die lijkt toe te staan dat een lege stack nog eenmaal gepopt kan worden zonder een fout te rapporteren.
...
Dit is niet mogelijk en als dit onverhoopt toch wordt gedaan, wordt de foutmelding gegenereerd ``Only Chuck Norris can divide by zero".
\newpage
...
\begin{tabular}{ | l | p{12cm} |}
    ...
    \begin{math} \theta \end{math} & Dit symbool staat voor de stack. \begin{math}\theta[0]\end{math} staat voor de eerste waarde van de stack. Het komt voor dat bovenste waarde van de stack gepopt wordt, waarna de onderliggende waardes automatisch omhoog schuiven. Dit wordt in de semantiekregels weergeven met \begin{math}\theta[1...]\end{math}. \\ \hline %MEER UITLEG OVER STACK GEBRUIK MET SYMBOLEN
    ...
    \begin{math} {\cal B} \end{math} &  Dit symbool staat voor Boolean en zal worden toegepast als er aan bepaalde condities moet worden voldaan om vervolgens een regel toe te passen. Het komt dat er meerdere condities moet worden voldaan dit wordt weergegeven met een $\land$. \\ \hline
...
\begin{tabular}[h]{l c r}
	...
	$[ass^s_{ns}]$	&	\AxiomC{$\langle $\&$, (\sigma, AV(\sigma), \rho, \theta, O) \rangle \rightarrow (\sigma, AV[\sigma \mapsto 0], \rho+\Delta, \theta, O \| $Stack is empty$ \| newline)$}
...
$[*_{ns}]$	&	\AxiomC{$\langle $*$, (\sigma, AV, \rho, \theta, O) \rangle \rightarrow  (\sigma, AV[\sigma \mapsto 0], \rho+\Delta, \theta, O \| $Stack is empty$ \| newline)$}
		  \DisplayProof & %$if {\cal B} _{[\theta[0] \neq undefined]s} == ff $
...
\begin{prooftree}
			\AxiomC{}
			\LeftLabel{$[\#_{ns}^n]$}
			\UnaryInfC{$\langle \#1, (\sigma, AV[\sigma \mapsto 3], \rho, \theta, O) \rangle \rightarrow s_1$}
...
\begin{prooftree}
			\AxiomC{}
			\LeftLabel{$[\#_{ns}^n]$}
			\UnaryInfC{$\langle \#1, (\sigma, AV[\sigma \mapsto 2], \rho+1, \theta, O||2||newline) \rangle \rightarrow s_5$}
...
\caption{Navigatie voorbeeld
		...
		\\$s_{4}: (1, AV[0 \mapsto 1, 1 \mapsto 0], 4\Delta, \theta, O \| $Stack is empty$ \| newline)$
}
%\centering
\begin{prooftree}
	\AxiomC{}
...
Dit voorbeeld laat zien hoe Foo een deling door 0 afhandeld, door de foutmelding te geven ``Only Chuck Norris can divide by zero" en vervolgens de huidige array waarde af te drukken.
...
\begin{prooftree}
	\AxiomC{}
	\LeftLabel{$[ass_{ns}^n]$}
	\UnaryInfC{$\langle \&10, s \rangle \rightarrow s_1 $}
		\AxiomC{}
		\LeftLabel{$[/^n_{ns}]$}
		\UnaryInfC{$\langle /0, s_1 \rangle \rightarrow s_2 $}
			\AxiomC{}
			\RightLabel{$[Print^i_{ns}]$}
			\UnaryInfC{$\langle \$i, s_2 \rangle \rightarrow s_3$}
			\RightLabel{$[Comp_{ns}]$}
		\BinaryInfC{$\langle /0\$i, s_1 \rangle \rightarrow s_3$}
		\RightLabel{$[Comp_{ns}]$}
	\BinaryInfC{$\langle \&10/0\$i, s \rangle \rightarrow s_3$}
\end{prooftree}
\end{figure}
...
Of door in de body van de loop, de celwaarde aan te passen, zoals beschreven in 4.2.
